...
<div class="content content-full content-boxed">
    ...
    <!-- New Post -->
    <form action="{{ URL::to('autenticar/usuario/guardar').'/'.$data->id }}" method="POST">
                ...
                <div class="row justify-content-center push">
                    <div class="col-md-10">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('rol') ? 'is-invalid' : '' !!}" name="rol" id="rol" {{ $errors->has('rol') ? 'aria-describedby="rol-error" aria-invalid="true"' : '' }}>
                                @foreach($tab_rol as $rol)
                                    <option value="{{ $rol->id }}" {{ $rol->id == $data->id_tab_rol ? 'selected' : '' }}>{{ $rol->de_rol }}</option>
                                @endforeach
                            </select>
                            @if( $errors->has('rol') )
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {!! $errors->has('nombre') ? 'is-invalid' : '' !!}" id="nombre" name="nombre" placeholder="Nombre / Apellido..." value="{{ empty(old('nombre'))? $data->nb_usuario : old('nombre') }}" {{ $errors->has('nombre') ? 'aria-describedby="nombre-error" aria-invalid="true"' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {!! $errors->has('correo') ? 'is-invalid' : '' !!}" id="correo" name="correo" placeholder="Correo Electrónico..." value="{{ empty(old('correo'))? $data->da_email : old('correo') }}" {{ $errors->has('correo') ? 'aria-describedby="correo-error" aria-invalid="true"' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('empresa') ? 'is-invalid' : '' !!}" name="empresa" id="empresa" {{ $errors->has('empresa') ? 'aria-describedby="empresa-error" aria-invalid="true"' : '' }}>
                                @foreach($tab_empresa as $empresa)
                                    <option value="{{ $empresa->id }}" {{ $empresa->id == $data->id_tab_empresa ? 'selected' : '' }}>{{ $empresa->nb_empresa }}</option>
                                @endforeach
                            </select>
