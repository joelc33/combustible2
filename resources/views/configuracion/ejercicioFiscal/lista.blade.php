...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/configuracion/ejercicio/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('configuracion/ejercicio/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/configuracion/ejercicio/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_ejercicio_fiscal as $key => $value)
                    <tr>
                        <td class="d-none d-sm-table-cell">{{ $value->id }}</td>
                        <td class="d-none d-sm-table-cell">{{ $value->fe_inicio }}</td>
                        <td class="d-none d-sm-table-cell">{{ $value->fe_fin }}</td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/configuracion/ejercicio/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/configuracion/ejercicio/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <a href="{{ url('/configuracion/ejercicio/editar').'/'. $value->id }}">
            ...
            {{ $tab_ejercicio_fiscal->appends(Request::only(['perPage','q']))->render() }}         
