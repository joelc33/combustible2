...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/configuracion/proceso/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('configuracion/proceso/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/configuracion/proceso/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_proceso as $key => $value)
                    <tr>
                        <td class="font-w600">{{ $value->id }}</td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_proceso }}</em></td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/configuracion/proceso/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/configuracion/proceso/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <a href="{{ url('/configuracion/proceso/editar').'/'. $value->id }}">
            ...
            {{ $tab_proceso->appends(Request::only(['perPage','q']))->render() }}         
